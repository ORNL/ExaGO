# AgentiGrid — Architecture Document
...
AgentiGrid is a project that brings large language model (LLM) intelligence into the simulation loop of power grid analysis. Rather than using an LLM merely as a conversational interface for setting up a single simulation run, AgentiGrid positions the LLM as an iterative decision-making agent that proposes parameter modifications, triggers simulations via ExaGO, interprets results, and decides subsequent actions — repeating this cycle until a user-defined goal is achieved.

This approach is motivated by a class of power system analysis tasks that are difficult to express as formal optimization problems but natural to describe in plain language: "find the load level at which the network becomes infeasible," "identify the weakest corridor under stress," or "find a dispatch that balances cost and voltage quality." A human expert would approach these tasks through iterative exploration — running a simulation, examining results, forming hypotheses, adjusting parameters, and repeating. AgentiGrid automates this expert workflow.

AgentiGrid was initially developed as a self-contained codebase and is currently in the process of being integrated into the ExaGO project, a large-scale power grid optimization toolkit developed at Oak Ridge National Laboratory and designed for execution on exascale computers such as Frontier. At this stage the AgentiGrid sources reside as a sibling subproject inside the ExaGO source tree; deeper integration with the ExaGO build system and runtime is ongoing work.
...
After the search loop completes, a shared utility module (`agentigrid/engine/goal_classifier.py`) builds a classification prompt and parses the LLM's structured JSON response. Both the CLI path (`AgentLoopController._finalize`) and the GUI path (`SessionManager.get_summary_analysis`) use this shared logic.
...
`agentigrid/cli.py` starts a daemon thread (`_start_stdin_listener`) when stdin is a TTY. Accepted commands: `pause`, `resume`, `stop`, `status`, `replace: <text>`, or bare text (augment).
...
|-----------|------|-------------|
| `ParetoCandidate` + `pareto_filter()` | `agentigrid/engine/pareto.py` | Non-dominated sorting over variant results. Feasible candidates dominate infeasible ones. Default objective: lowest generation cost. Multi-objective: uses registered objective directions. |
| `VariantResult` + `ExploreCache` | `agentigrid/engine/explore.py` | Data structures for explore results. `ExploreCache` holds all variant results between explore and select actions. |
| `format_variant_results()` | `agentigrid/engine/explore.py` | Formats variant comparison table for LLM prompt with Pareto ★ markers. |
...
- **Conda compatibility:** Runs in exago312 environment (ExaGO) and agentigrid environment (AgentiGrid, Python 3.11).
...
- **Sweep infeasibility: `status` is the only certified source of truth.** OPFLOW reports `DID NOT CONVERGE` for all infeasible candidates regardless of internal cause. Scalar iterate metrics (V_min, V_max, line loading, violation count) come from the solver's last uncertified iterate — they are diagnostic hints, not confirmed constraint causes. The `_infeasible_reason` engine function takes only `convergence_status` as input; uncertified metrics are shown in a separately-labelled table group but never used to classify the reason. Historical journals that stored heuristic labels (e.g., "line overload") are rendered correctly by deriving the display reason from `status` at render time, not from the stored `reason` string.
- **Near-optimal sweep candidates must be flagged, not ranked.** In a cost-minimization sweep, IPOPT's noise floor (~$5/h) means a $1–2 cost difference between two buses carries no engineering significance. Without explicit flagging, the report declares a false unique optimum. The `report.near_optimal_abs_tol` config field (default $5/h) controls when the top-2 gap triggers a caveat; `report.cost_min_top_k` (default 10) controls the ranking table size. Both fields are surfaced in the Streamlit UI and the PDF report.
- **LLM-facing sweep text must be token-bounded, not full-table.** The full per-candidate collect-all table scales linearly with network size: ~18,658 tokens for ACTIVSg200 (56 buses), ~100,903 tokens for ACTIVSg2000. At 3000+ buses it approaches the context window. The solution is threshold-gated: when `candidate_count <= search.sweep_full_table_threshold` (default 250) the full table is emitted unchanged (preserving ACTIVSg200 baselines); above the threshold, a bounded summary is emitted: header counts + complete feasible bus list + infeasible buses grouped by reason (O(n_reasons) lines, not O(n_infeasible)) + top-N ranked by primary objective + aggregate stats + a journal pointer line. Token cost is then O(top_n + n_feasible + n_reasons) instead of O(n_candidates). `search.sweep_llm_top_n` (default 25) controls the ranked block size. The journal always receives the complete candidate_summaries regardless of view mode.
- **OPFLOW sweep concurrency vs BLAS oversubscription.** Running N OPFLOW subprocesses in parallel while each subprocess spawns M BLAS threads creates N×M threads competing for the same physical cores. Pinning each worker's BLAS thread count to 1 (via `OMP_NUM_THREADS` and friends) when `workers > 1` keeps the total thread count at N and fully utilises all cores without thrashing. The `search.sweep_max_workers` config field (0 = auto) and per-worker thread pinning in `SimulationExecutor.run()` implement this.
- **Boundary (hosting-capacity) search is a per-candidate bisection, not a per-candidate verdict.** "How much can bus X host?" needs the *maximum feasible injection*, not a single feasible/infeasible test. The `sweep` action's `mode: "boundary"` runs an exponential-bracket-then-bisect on injection magnitude per bus. The outer loop over buses is embarrassingly parallel (`SimulationExecutor.map_callables`, which runs arbitrary candidate callables while each inner solve stays single-threaded via `run(thread_limit=…)`); the bisection within a bus is sequential (each probe depends on the previous). The whole boundary sweep is ONE LLM turn performing N bisections — it must not consume N iterations of the LLM budget.
- **Hosting capacity must scale load on a constant-PF ray, not P alone.** Adding active load with no reactive component silently improves the power factor and overstates capacity. The load boundary mode scales `Qd` with `Pd` along `ΔQ = ΔP · tan φ`, where `tan φ` defaults to the system-average `ΣQd/ΣPd` (configurable per-PF or unity). For generators, fixed injection (`Pmin = Pmax = ΔP`) is mandatory — a dispatchable unit is zeroed by the OPF, making the hosting test vacuous — with reactive output free within `±boundary_gen_q_frac · ΔP` so the located limit is genuinely the MW boundary.
- **The boundary is the convergence boundary; non-convergence is the infeasible signal.** Under OPFLOW the V-band and Rate A are in-solve hard constraints, so the largest converging injection IS the hosting capacity. Non-convergence caps the bisection (standard for OPF hosting capacity), with a reported caveat that a numerically-failing probe would understate capacity.
- **Binding-constraint identification is activity-based, not value-based, and is a diagnostic label distinct from the certified capacity.** The `binding_constraint` label attached to each `max_feasible_mw` is computed at the max-feasible point and never affects the capacity number. ExaGO OPFLOW exposes **line-flow Lagrange multipliers** (`mult_Sf`/`mult_St`, nonzero exactly when a flow limit is active) but **not** voltage-bound shadow prices (the bus table carries only power-balance multipliers). So identification is hybrid: (1) **thermal** — the genuinely-active line with the largest |multiplier| when duals are present, else the *globally* most-loaded line(s) within `binding_eps` (0.5%) of the system max, computed from **both ends** `max(|Sf|,|St|)/Slim` (an earlier bug used `Sf` only and mis-named local lines at 94–97% while the system was at 100%); (2) **voltage** — a bound binds only if its margin *collapsed* relative to the base solve (clearly positive in base, ~0 at the boundary), which excludes pinned PV setpoints (e.g. a bus regulating to Vmax=1.10 in every solve, previously blamed by the value-based `metric == limit` test); (3) when both a thermal line and a voltage bound are newly active, **both** are reported. The base-case solved result is threaded into the identifier for the margin-vs-base comparison. Historical journals that predate this fix cannot be retroactively corrected without re-running.
- **Dispatchable-generator siting needs a load-bearing cost curve, not a cosmetic one.** Min-cost location for a *dispatchable* unit (prompt 13) is the deliberate exception to forced injection: the OPF must choose the unit's output, so the unit needs a realistic cost curve. With zero cost it dispatches to max everywhere (free energy) and the location ranking is meaningless; with too-high cost it never dispatches and every location looks identical to base. The default curve is the **case median** of existing model-2 polynomials (mid-merit), so the unit partially dispatches and locations differentiate (`search.added_gen_cost_strategy`). The per-candidate dispatched Pg is recorded as a diagnostic — a unit dispatching ~0 MW is not helping at that location.
- **Fixed-injection vs dispatchable must be chosen from the prompt wording, never defaulted.** "Hosting capacity / forced output" → fixed injection (`Pmin = Pmax`); "economic dispatch / minimize cost / let the unit choose" → dispatchable (`Pmin = 0, Pmax = cap`) with a cost curve. These are different questions (e.g. a forced 160/40 split vs an optimized 200/200 dispatch) and silently picking the wrong mode flips the answer — the system prompt (rule X.1) makes the LLM map phrasing → mode and name the mode back.
- **Custom sweep metrics/predicates are a named registry of verified primitives, not LLM-synthesized code.** This is the Phase-1 trusted-primitive approach (`engine/sweep_metrics.py`): the LLM selects a `metric` / `feasibility_predicate` *by name*; the registry is extensible via `register_metric` / `register_predicate` for later capabilities. `max_delta_v` (worst system-wide voltage step on switching, needs a one-time base solve) and `reactive_adequacy` (feasible OPF at forced P=Pmax, Q=Qmax) generalize the sweep beyond cost + standard V-band/loading feasibility. When neither is specified the default path is byte-identical to before, so existing baselines are preserved.
- **Custom metrics are certified-gated — same rule as the binding-constraint identifier.** A metric read from a candidate's solved state is trustworthy only if that candidate *converged*; the shared `_is_certified(opflow)` gate (status starts with `CONVERGED`) decides this. For non-converged candidates `metric_value` is set to `None`, so the LLM's max/min reduction and the report operate over certified candidates only and never elevate an uncertified iterate to the headline. (Evidence: prompt-15 originally reported 0.09 p.u. at a *non-converged* bus 77 — its iterate showed 293.8% loading — when the correct feasible maximum was 0.032 at bus 139.) Note on semantics: under OPFLOW `max_delta_v` is the change between two cost-optimal voltage profiles (the solver re-dispatches reactive support), a steady-state sensitivity, not a physical energization transient; it is taken over all buses including the connection bus.
- **Best-feasible summary aggregation must descend into sweep variants.** Sweep iterations carry no scalar `objective_value` (they journal as `SWEEP`), so a naive iteration-level minimum collapses to the base case and reports it as the optimum. `SearchJournal.summary_stats` therefore descends into each cost sweep's `explored_variants` and takes the global minimum feasible per-candidate cost (returning `best_bus` alongside `best_objective`/`best_iteration`). It stays goal-aware: boundary sweeps (variants carry `max_feasible_mw`) and custom-metric sweeps (variants carry `metric_name`) are skipped, and the descent only runs for cost-style goals. (Evidence: prompt-13 reported the base case $27,557.57 (iter 0) instead of the sweep optimum $27,367.73 at bus 181.)
- **`reactive_adequacy` forces Q, and records it for audit.** The predicate's intent is "Qmax *while at* Pmax", so the mutation pins `Qmin == Qmax == Qmax_target` (forced, not merely upper-bounded). The added unit's solved reactive output is recorded as `dispatched_q` (mirroring `dispatched_pg`) and surfaced in the reactive-adequacy table — at every certified/adequate bus it should equal the Qmax target, making the forcing verifiable from results.
- **Identical sweeps are de-duplicated within a session.** Sweeps are deterministic, so re-running an identical request wastes 2–3× the solves (prompt 13 ran the same sweep three times). A system-prompt rule tells the LLM not to re-run an identical sweep; as a deterministic backstop the controller caches results by a sweep signature (mode + entity + mutation + candidate set + metric/predicate + params, excluding free-text description/reasoning) and serves a cache hit from the journal with a "cached — not re-solved" note instead of re-solving. A sweep with any changed parameter has a different signature and still executes.
