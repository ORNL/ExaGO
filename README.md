...
- [OPFLOW](docs/web/opflow.md) solves an AC optimal power flow either on CPU or GPU
...
ExaGO<sup>TM</sup> applications are interfaced with the following optimization solver packages:
...
- [HiOp](https://github.com/LLNL/hiop) is an HPC package for optimization. ExaGO interfaces with two of its solvers -- an interior-point method solver (HIOPSPARSE) and a primal decomposition two-level solver HiOp PriDec.
...
- [test_add.md](docs/web/test_add.md) markdown file for information on adding tests (outdated)
...
- [README.md](docs/devcontainer/README.md) for our devcontainer configuration information (codespace support coming soon)
- [exago_policy_compatibility](docs/exago_policy_compatibility.md) for xSDK compatibility guidelines, and ways to enforce compliance
- [python_bindings.md](docs/python_bindings.md) for documentation about our Python bindings
...
- If you are using a devcontainer with VSCode, the following tutorials are provided:
    - [tutorial.ipynb](docs/devcontainer/tutorial.ipynb) for basic configuration information and I/O
...
- Otherwise, you can check out our more in depth application tutorials in the `tutorials` subdirectory:
    ...
    - [demo2.ipynb](tutorials/demo2.ipynb) run SOPFLOW on many ranks using MPI, and visualize output
